...
const translations: Record<Language, TranslationContent> = {
  es: {
    ...
    privacyText:
      "Tus datos siempre están protegidos con Coin Sin Limited. Al completar este formulario, aceptas recibir nuestros correos electrónicos de marketing.", // Modified
    ...
    step2Description:
      "Como en cualquier negocio, necesitas capital inicial. La ventaja de la plataforma Coin Sin Limited es que solo requiere una inversión inicial modest. Simplemente deposita 250€ o más para empezar a ganar dinero.",
  ...
  en: {
    ...
    privacyText:
      "Your data is always protected with Coin Sin Limited. By completing this form, you agree to receive our marketing emails.", // Modified
    ...
    feature2:
      "The supercomputer can calculate millions of variants every second and predict trends with maximum accuracy.",
    ...
    featureCommunityText:
      "Join our prestigious Coin Sin Limited community and gain privileged membership. Please note that, due to system capacity limitations, we can only send invitations to a select number of users. Take advantage of this opportunity to solve your financial problems once and for all.",
  ...
  ca: {
    ...
    motivationalText:
      "TENS DRET A LA INDEPENDÈNCIA FINANCERA, I NO ÉS UN PRIVILEGI. ÉS EL TU DRET, INDEPENDENTMENT DE LA TEVA EDAT, ASSOLIMENTS I ESTATUS SOCIAL.",
    ...
    privacyText:
      "Les teves dades sempre estan protegides amb Coin Sin Limited. En completar aquest formulari, acceptes rebre els nostres correus electrònics de màrqueting.", // Modified
    ...
    focusOnPlatformText2:
      "A més, la intel·ligència artificial opera en el mercat les 24/7 ara. Significa coneixement de la situació global en temps real, acumulació de coneixements sobre patrons i creació immediata d'estratègies rentables. La plataforma Coin Sin Limited ofereix una rendibilitat eficaç d'assolir per als humans. És qüestió de temps que les màquines assumeixin plenament aquesta funció.",
    focusOnPlatformText3:
    ...
    investSmartText2:
      "La plataforma d'inversió Coin Sin Limited le permet fer-ho sense perdre temps estudiant el mercat de divises digitals. Pot començar a invertir avui mateix en països com el Canadà, Austràlia i altres. L'èxit està precalculat, i tot el que necessita és el desig d'unir-se.",
    ...
    tool3: "Treballar en el mercat con una àmplia diversificació de la cartera i una reducció paral·lela del risc.",
    ...
    notScamText4:
      "Coin Sin Limited ofereix màxims beneficis y guanys sense risc, i presenta proves de no fraud. També trobaràs opinions sobre Coin Sin Limited a continuació.",
    ...
    featureAutoTradingText:
      "Experimenta la comoditat de la nostra funció de trading automàtic, que et permet generar guanys sense esforç, fins i tot quan no estiguis al teu lloc de treball. No es requereix experiència comercial! Simplement fes la teva inversió inicial i observa com el saldo del teu compte creix constantment.",
    ...
    step2Title: "DEPOSITA €250 O MÁS",
    step2Description:
      "Com a qualsevol negoci, necessites capital inicial. L'avantatge de la plataforma Coin Sin Limited és que només requereix una inversió inicial modesta. Simplement diposita 250€ o més per començar a guanyar diners.",
    step3Title: "RESTA SINTONITZAT SUL TU TELÈFON... POTRIES REBRE UNA TRUCADA!",
    step3Description:
      "Després de fer un pagament, el nostre gestor es posarà en contacte amb tu per confirmar-ho tot i activar el teu compte. Si tens alguna pregunta, el gestor et proporcionarà respostes detallades per ajudar-te. Tenere presente que la trucada podria provenir d'un número no identificat.",
    faqTitle: "PREGUNTES",
    ...
    finalSectionText:
      "Agisci ora fornendo il tuo nome completo e la tua email nel modulo sottostante, e sblocca l'opportunità più eccezionale ed exclusiva per generare un reddito sostanziale senza sforzo. Lascia che l'IA si ocupi del lavoro duro mientras tú recoges beneficios tangibili al instante. ¡No te lo pierdas!",
    footerCompanyInfo:
...
const testimonials = [
  ...
  },
  {
    ...
    text: {
      ...
      it: "Dopo anni di duro lavoro, cercavo un modo per assicurarmi la pensione. L'automazione e la precisione dell'IA mi permettono di generare profitti senza stress. È la migliore decisione finanziaria che abbia mai preso.",
    },
  },
  {
    id: 3,
  ...
  },
  {
    ...
    text: {
      ...
      ca: "Sempre vaig ser escèptic con les inversions en línia, però Coin Sin Limited em va demostrar el contrari. La transparència i els resultats són reals. He diversificat la meva cartera i els meus guanys superen amb escreix les meves expectatives inicials. Una plataforma de confiança!",
  ...
  },
  {
    ...
    text: {
      ...
      ca: "Mai vaig pensar que a la meva edat podria entendre el món de les criptomonedes, però Coin Sin Limited ho va fer possible. La plataforma és intuitiva i els resultats són consistents. És una bendición per a la meva economia familiar!",
...
const countryCodes = [
  ...
  { id: "KZ", code: "+7", flag: "🇰🇿", country: "Kazajistán" },
  ...
  { id: "LT", code: "+370", flag: "🇱🇹", country: "Lituania", name: "Lituania" },
...
export default function CryptoLanding() {
  ...
  useEffect(() => {
    ...
    // Lógica para obtener el código de país basado en la IP
    // Este useEffect se ejecutará solo una vez al montar el componente.
    const fetchCountryCode = async () => {
      console.log("[CryptoLanding] Intentando detectar código de país por IP...")
      try {
        ...
        if (data.countryCode) {
          ...
          if (detectedCode) {
            // Solo actualiza si el código detectado es diferente al actual o al predeterminado
            if (formData.countryCode === "+1-809" || formData.countryCode !== detectedCode.code) {
              setFormData((prev) => ({ ...prev, countryCode: detectedCode.code }))
              console.log(`[CryptoLanding] Código de país actualizado a: ${detectedCode.code}`)
            } else {
              console.log("[CryptoLanding] Código de país ya establecido o detectado es el mismo.")
            }
          } else {
            console.log(`[CryptoLanding] Código de país detectado (${data.countryCode}) no encontrado en la lista.`)
          }
        } else {
          console.log("[CryptoLanding] API no devolvió un código de país.")
        }
      } catch (error) {
        console.error("[CryptoLanding] Error al obtener el código de país por IP:", error)
      }
    }
  ...
  // Formatear el tiempo de cuenta regresiva
  ...
  // Calcular el retorno basado en la inversión y los días
  ...
  const nextTestimonial = () => {
    setCurrentTestimonial((prev) => {
      // Asegura que maxIndex se calcule correctamente para el número de testimonios y elementos mostrados
      const maxIndex = testimonials.length - 3 // Mostrando 3 testimonios a la vez
  ...
  const validatePhoneNumber = (phoneNumber: string) => {
    ...
    // Check for repeating digits (e.g., 11111, 22222) if length is significant
    if (digitsOnly.length >= 5 && /^(\d)\1+$/.test(digitsOnly)) {
      return "Por favor, introduce un número de teléfono válido (evita dígitos repetidos)."
    }
    // Check for common sequential patterns (e.g., 123456, 654321)
    // This checks for sequences of 5 or more digits
    ...
    if (sequentialPatterns.some((pattern) => digitsOnly.includes(pattern.source))) {
      // Use .includes for partial matches
      return "Por favor, introduce un número de teléfono válido (evita secuencias)."
    }
    // Basic length check (adjust as per actual requirements for phone numbers)
    ...
    <div className="min-h-screen relative overflow-hidden bg-blue-950">
      ...
      <div className="relative z-10" aria-label="Background image of cryptocurrency charts">
        ...
        <div className="relative z-20 min-h-screen flex flex-col">
          ...
          {/* Header */}
          {/* Modificado para que el contador dinámico esté más a la izquierda */}
          <header className="relative z-20 grid grid-cols-[auto_1fr_auto] items-center py-1 px-2 sm:py-2 sm:px-4">
            ...
            {/* Added justify-self-center to center it within the 1fr column */}
            ...
            <div className="w-full max-w-6xl">
              ...
              <div className="rounded-2xl p-4 sm:p-6 shadow-2xl border border-blue-400/20 animate-in slide-in-from-bottom-5 duration-700 glowing-form-shadow">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 items-center">
                      ...
                      <div className="relative bg-transparent rounded-xl overflow-hidden glowing-form-shadow">
                        <div className="relative w-full" style={{ paddingTop: "56.25%", overflow: "hidden" }}>
                          ...
                          <iframe
                            ...
                            // Removed style={{ transform: "translateY(-10%) scale(1.2)" }} to show full video
                  ...
                  <div
                    ...
                    className={`space-y-4 ${registrationState.success ? "flex flex-col items-center justify-center h-full" : ""
                      }`}
                  >
                        ...
                        <form className="space-y-3" action={registrationAction}>
                          ...
                          <div className="relative group animate-in slide-in-from-left-3 duration-500 delay-350">
                            ...
                            <Input
                              ...
                              required
                              min="1950-01-01" // Mínimo 1 de enero de 1950
                              max="2007-12-31" // Máximo 31 de diciembre de 2007
                              className="w-full bg-white/95 backdrop-blur-sm border-0 h-10 text-gray-900 placeholder:text-gray-500 pl-10 pr-10 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 focus:scale-[1.02] focus:bg-white group"
                            />
                          ...
                          <div className="animate-in slide-in-from-bottom-3 duration-500 delay-500">
                            ...
                            {/* Error al enviar el correo aparece abajo del botón de registro */}
                            {registrationState.errors?._form && registrationState.errors._form.length > 0 && (
                              <p className="text-red-400 text-xs text-center mt-2">
                                {registrationState.errors._form[0]}
                              </p>
                            )}
                          </div>
                          ...
                          <div className="flex items-start gap-3 text-xs text-white animate-in fade-in-50 duration-500 delay-600">
                            ...
                            <label
                              ...
                              className="leading-relaxed cursor-pointer hover:text-orange-200 transition-colors duration-200"
                            >
                              <Shield className="w-3 h-3 inline mr-1 text-orange-400 align-middle" /> {t.termsText}{" "}
                            </label>
                          </div>
              ...
              <div className="text-center mt-8 sm:mt-10">
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-1 sm:mb-2 leading-tight">
                  {" "}
                  {t.mainTitle} <br /> <span className="text-orange-400">{t.mainTitleHighlight}</span>{" "}
                </h1>
                    ...
                    <div className="animate-in slide-in-from-right-5 duration-700">
                      <div className="bg-blue-600/90 backdrop-blur-sm rounded-2xl p-6 shadow-2xl border border-blue-400/20">
                        ...
                        <form className="space-y-4" action={registrationAction}>
                          ...
                          {/* Error al enviar el correo aparece abajo del botón de registro */}
                          {registrationState.errors?._form && registrationState.errors._form.length > 0 && (
                            <p className="text-red-400 text-xs text-center mt-2">{registrationState.errors._form[0]}</p>
                          )}
